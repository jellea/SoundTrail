...
L.GPX = L.FeatureGroup.extend({
  ...
  // If 'wpt' element parse it's name
  ...
  parse_trkseg: function(line, xml, options, tag) {
    ...
    for (var i = 0; i < el.length; i++) {
      ...
      var tracktime = new Date(el[i].getElementsByTagName('time')[0]['textContent']);
      if (i == 0){
        starttime = tracktime
      }
      var diff = tracktime - starttime;
      L.route[diff/1000] = {
        'lat': el[i].getAttribute('lat'),
        'lon': el[i].getAttribute('lon'),
        'time': tracktime
      };
      ll.meta = {};
